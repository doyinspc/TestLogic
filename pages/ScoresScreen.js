...
class ScoresScreen extends React.Component{
  ...
  relocate = () =>{
    ...
    if(value && value > 0)
    {
     this.props.navigation.navigate('ScoreSheetScreen', {val:value})
    }
  }
  ...
  async componentDidMount() {
    ...
    if(!isLoading && test_data && Object.keys(test_data).length > 0){
        ...
        test_array[7] = ['Timer', settings[1]];
        test_array[8] = ['Answer', settings[2]];
  ...
  getFutureTime = () =>{
    ...
    let fhours = newTime/(365 * 60 * 60);
    let fhour = Math.floor(fhours);
    let fmins = (newTime - (fhour * 60 * 60)) / 60;
    let fmin = Math.floor(fmins);
    let fsec = newTime - ((fhour * 60 * 60) + (fmin * 60)) ;
    let tdate = new Date();
    tdate.setTime(newTime);
    this.setState({futureTime: `${tdate.getHours()} : ${tdate.getMinutes()} : ${tdate.getSeconds()}` });
  }
  ...
  updateIndex = (selectedIndex) =>{
    ...
    {
        this.props.navigation.navigate('ScoreScreen', {'testID':testID });
    }
       ...
       {!fontLoaded && isLoading ?  
        ...
        <View style={{flex: 10}}>
          <ScrollView>
           ...
           <View style={{ flex:1, flexDirection:'column', margin:30, padding:20, bottom:0, justifyContent:'center' }}>
